    ...
    try {
      ...
      if (!checkUserDetails) {
        ...
        const htmlContent = this.sendEmailServices.template.invite_team_user
          ...
          .replace(
            '[RegisterLink]',
            `${config.get('user_qalarsmm.mail_url')}/register`
          )
          .replace('[teamname]', `${teamInfo.team_name ?? ''}`)
          .replace('[inviteduser]', `${req.body.userScopeName}`);
        let emailDetails = {
          subject: `qalarsmm Team Invitation By ${req.body.userScopeName}`,
          toMail: Email,
          htmlContent,
        };
        ...
        return SuccessResponse(
          res,
          `User not found in qalarsmm. Sent a invitation mail to join your team.`
        );
      }
      ...
      if (config.get('notification_qalarsmm.status') == 'on') {
