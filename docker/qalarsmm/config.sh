...
while [[ $(echo $config | jq '.mailTitles') != 'null' ]];do
    ...
    config=$(echo $config | jq --arg a "Invitation from qalarsmm to Add Account" '.mailTitles.Invitation_user = $a')
...
while [[ $(echo $config | jq '.mysql') != 'null' ]];do
    config=$(echo $config | jq --arg a "qalarsmm-mysql" '.mysql.host = $a')
...
while [[ $(echo $config | jq '.mongo') != 'null' ]];do
    ...
    config=$(echo $config | jq --arg a "qalarsmm-mongo" '.mongo.host = $a')
...
# qalarsmm twitter
while [[ $(echo $config | jq '.qalarsmmTwitterhandler') != 'null' ]];do
    config=$(echo $config | jq --arg a "$qalarsmm_TWITTER_HANDLER" '.qalarsmmTwitterhandler = $a')
    break;
done
# aMember
...
while [[ $1 == "/usr/qalarsmm/app/qalarsmm-api/User/config/development.json" ]];do
...
# admin
while [[ $1 == "/usr/qalarsmm/app/qalarsmm-api/Admin/config/development.json" ]];do
...
sed -i "s;local.qalarsmm.com;socio.${BASE_DOMAIN}${port};g" $1
sed -i "s;localhost;socio.${BASE_DOMAIN}${port};g" $1
sed -i "s;notifyv5.qalarsmm.com;socio-notifications.${BASE_DOMAIN}${port};g" $1
