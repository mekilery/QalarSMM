#!/bin/bash
# mysql
cd /usr/qalarsmm/app/qalarsmm-api/Common/Sequelize-cli/config/
...
config=$(echo $config | jq --arg a "qalarsmm-mysql" '.development.host = $a')
rm -f config.json
...
cd /
./config.sh "/usr/qalarsmm/app/qalarsmm-api/User/config/development.json"
./config.sh "/usr/qalarsmm/app/qalarsmm-api/Update/config/development.json"
./config.sh "/usr/qalarsmm/app/qalarsmm-api/Feeds/config/development.json"
./config.sh "/usr/qalarsmm/app/qalarsmm-api/Notification/config/development.json"
./config.sh "/usr/qalarsmm/app/qalarsmm-api/Publish/config/development.json"
./config.sh "/usr/qalarsmm/app/qalarsmm-api/Admin/config/development.json"

# rename config files
cd /usr/qalarsmm/app/qalarsmm-api
cd ./User/config
...
while [ ! -e "/data/db.init" ];do
    ...
    cd /usr/qalarsmm/app/qalarsmm-api/Common/Sequelize-cli
...
while [ ! -e "/ssl/cert.pem" ];do
    ...
    openssl req -subj "/CN=*.$BASE_DOMAIN/O=qalarsmm/C=US" -new -newkey rsa:2048 -sha256 -days 365 -nodes -x509 -keyout /ssl/key.pem -out /ssl/cert.pem
    break;
done
...
cd /usr/qalarsmm/app/qalarsmm-api/User && npm run swagger
