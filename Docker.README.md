...
NOTE 2: If you would like to change the subdomains, be sure to change the URL scheme in `docker/qalarsmm/init.sh`, `docker/qalarsmm/config.sh`, `docker/qalarsmm-web/init.sh` and in `docker/nginx/nginx-qalarsmm.conf`.
...
```bash
docker logs qalarsmm
```

Once services are running, we can check the logs using:
```bash
docker exec -it qalarsmm su-exec qalarsmm pm2 logs
```
CTRL+C to exit
...
NOTE: When serving https on a port other than 443, the http redirect will no longer work. You will either have to modify the redirect in `docker/nginx/nginx-qalarsmm.conf` or handle it with your own reverse proxy(if you're using your own reverse proxy AND non standard ports I'd suggest handling the redirect there or leaving the http port out entirely).
